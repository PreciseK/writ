...
const DEFAULT_CUSTOMERS = [
  ...
  { id: 4, date: '10/12/2023', status: 'Cancelled', statusVariant: 'danger',  name: 'Theo Ng',       avatar: THEO_AVATAR,    revenue: '$19.99' },
];
...
export default function CustomersTableCard({
  ...
  return (
    <section
      className={cn(
        'relative w-full overflow-hidden rounded-2xl border border-border/60 shadow-md bg-background',
        className
      )}
      aria-label={title}
    >
          ...
          <tbody>
            {customers.map((customer, idx) => (
              ...
              >
                ...
                <td><Badge variant={customer.statusVariant}>{customer.status}</Badge></td>
                <td>
      ...
      <div className="flex items-center justify-between border-t border-border/60 p-4 text-xs text-muted-foreground">
        <span>Showing <strong>{customers.length}</strong> {customers.length === 1 ? 'row' : 'rows'}</span>
