...
export function PulseFitHero({
  ...
  const hasHeader = logo || (navigation && navigation.length > 0) || ctaButton;
  ...
  return (
    ...
    >
      {/* Header — only rendered when logo / navigation / ctaButton are provided */}
      {hasHeader && (
        <motion.header
        ...
        >
          {/* Logo */}
          {logo && (
            <div style={{ fontWeight: 700, fontSize: "24px", color: "#1a1a1a" }}>
              {logo}
            </div>
          )}
            ...
            <nav className="hidden lg:flex flex-row items-center gap-8" aria-label="Main navigation">
              {navigation.map((item, index) => (
                <button
                  ...
                  style={{ fontSize: "16px", fontWeight: 400, color: "#4a5568" }}
                >
                  {item.label}
                  {item.hasDropdown && (
