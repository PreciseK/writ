...
const createParticles = (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, text: string, textX: number, textY: number, font: string, color: string, alignment: "left" | "center" | "right") => {
  ...
  // Auto-scale font down if text would overflow canvas width
  const maxAllowedWidth = canvas.width * 0.92;
  let measuredWidth = ctx.measureText(text).width;
  if (measuredWidth > maxAllowedWidth) {
    const currentSize = parseFloat(font.split("px")[0].split(" ").pop() || "50");
    const scaledSize = Math.floor(currentSize * (maxAllowedWidth / measuredWidth));
    ctx.font = font.replace(/(\d+(?:\.\d+)?)px/, `${scaledSize}px`);
  }
...
const resetParticles = (particles: Particle[]) => {
  particles.forEach(particle => { particle.x = particle.originalX; particle.y = particle.originalY; particle.opacity = particle.originalAlpha; particle.speed = 0; particle.velocityX = 0; particle.velocityY = 0; });
};
...
function transformValue(input: number, inputRange: number[], outputRange: number[], clamp = false): number {
  ...
  let result = outputMin + (input - inputMin) / (inputMax - inputMin) * (outputMax - outputMin);
